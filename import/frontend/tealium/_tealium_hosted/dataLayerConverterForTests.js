/**
 * This is the exact same script except instead of the line:
 * window.teal = window.teal || {};
 * We have the line:
 * let teal = {};
 *
 * We then wrap all of this in one function and return `teal`
 *
 * This is because we can't load the script on the page in Cypress so we need to do the following:
 * Cypress.on('window:before:load', (win) => {
 *   win.teal = createTestVersion();
 * })
 *
 * Which will then allow the adapter to behave as expected
 */

export const createTestVersion = () => {
  // Tealium datalayer converter
  // should be loaded as a tealium extension in the Preload scope
  // https://community.tealiumiq.com/t5/iQ-Tag-Management/Data-Layer-Converter/ta-p/17581
    ...
    if (typeof teal.replace_keys_list === "undefined") {
      ...
      teal.replace_keys_list.forEach(function (name) {
        ...
        teal.replace_keys_regex[name] = new RegExp(
          "[" + teal.nested_delimiter + "]?" + name + "[" + teal.nested_delimiter + "]?",
          "g"
        );
